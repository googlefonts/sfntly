...
public abstract class LookupTableOld extends OTSubTable {
  private final int flags;

  protected LookupTableOld(ReadableFontData data, boolean dataIsCanonical) {
    super(data, dataIsCanonical);
    flags = readLookupFlags(data);
  }

  @Override
  public abstract Builder<? extends LookupTableOld> builder();
  ...
  static abstract class Builder<T extends LookupTableOld> extends OTSubTable.Builder<T> {
    ...
    public Builder<T> setMarkFilteringOffset(int offset) {
      if (offset > 0) {
        flags = LookupTableOld.setFlag(flags, FLAG_USE_MARK_FILTERING_SET);
        markFilteringSetOffset = offset;
      } else {
        flags = LookupTableOld.clearFlag(flags, FLAG_USE_MARK_FILTERING_SET);
      }
      return this;
    }
