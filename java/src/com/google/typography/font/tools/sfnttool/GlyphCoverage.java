...
public class GlyphCoverage {
  ...
  public static List<Integer> getGlyphCoverage(Font font, String string) {
    ...
    CMapTable.CMap cmap = getBestCMap(cmapTable);
  ...
  private static void touchGlyph(Font font, Set<Integer> coverage, int glyphId) {
    if (!coverage.contains(glyphId)) {
      ...
      GlyphTable.Glyph glyph = getGlyph(font, glyphId);
      if (glyph != null && glyph.glyphType() == GlyphTable.GlyphType.Composite) {
  ...
  private static CMapTable.CMap getBestCMap(CMapTable cmapTable) {
    for (CMapTable.CMap cmap : cmapTable) {
      if (cmap.format() == CMapTable.CMapFormat.Format12.value()) {
        return cmap;
      }
    }
    for (CMapTable.CMap cmap : cmapTable) {
